...
type ColMap = { date: number; amount: number; desc: number; hasHeader: boolean };

// §6-4 主要数社のプリセット（列の並びの目安。最後は手マッピングで吸収）
const PRESETS: { label: string; map: ColMap }[] = [
  { label: '三井住友カード風（日付, 摘要, 金額）', map: { date: 0, desc: 1, amount: 2, hasHeader: true } },
  { label: '汎用（日付, 金額, 摘要）', map: { date: 0, amount: 1, desc: 2, hasHeader: true } },
  { label: '楽天カード風（利用日, 利用先, …, 金額）', map: { date: 0, desc: 1, amount: 4, hasHeader: true } },
];
...
function guessColMap(rows: string[][]): ColMap {
  ...
  let dateCol = 0;
  let bestDate = -1;
  for (const s of stats) if (s.dateRate > bestDate) { bestDate = s.dateRate; dateCol = s.i; }

  // 金額列: 数値で「金額らしい（100以上が多い）」列。手数料(ほぼ0)や回数(小さい固定値)は除外される。
  // 利用金額/支払総額のように複数の金額列があるときは、より手前（実取引額のことが多い）を選ぶ。
  const moneyCols = stats.filter((s) => s.i !== dateCol && s.amtRate >= 0.6 && s.moneyFrac >= 0.5);
  ...
  if (moneyCols.length > 0) {
    ...
    let best = -1;
    amountCol = -1;
    for (const s of stats) if (s.i !== dateCol && s.amtRate > best) { best = s.amtRate; amountCol = s.i; }
  ...
  for (const s of stats) {
    ...
    const score = s.fillRate * s.fillRate * s.textRate * s.distinctRatio * Math.min(s.avgLen, 20);
    if (score > bestDesc) { bestDesc = score; descCol = s.i; }
  }
  if (descCol < 0) descCol = [0, 1, 2].find((i) => i !== dateCol && i !== amountCol) ?? 1;
...
export default function ImportPage() {
  ...
  const [colMap, setColMap] = useState<ColMap>({ date: 0, desc: 1, amount: 2, hasHeader: true });
  ...
  const runGrouping = () => {
    ...
    const dataRows = files.flatMap((f) => (colMap.hasHeader ? f.rows.slice(1) : f.rows));
    ...
    if (txs.length === 0) {
      setError('指定した列から有効な明細を読み取れませんでした。列マッピングを見直してください。');
      setCandidates([]);
      return;
    }
  ...
  const importSelected = async () => {
    ...
    for (const c of chosen) {
      ...
      const priceText =
        c.cadence === 'irregular' ? `約${amountText}` : `約${amountText}（${cadenceLabel(c.cadence)}）`;
  ...
  return (
    <div className="space-y-5">
      <header>
        <h1 className="text-xl font-bold text-slate-800">カード明細から定期課金を取り込む</h1>
        <p className="mt-1 text-sm text-slate-600">
          明細を店舗・サービス別にまとめます。どれをサブスクとして取り込むかは一覧から目視で選べます。
          解析は<strong>すべてこのブラウザ内</strong>で行います。明細データは外部に送信されません（LLM・外部APIも不使用）。
        ...
        <div className="mt-3 rounded-md border border-slate-200 bg-slate-50 p-3 text-sm text-slate-600">
          このステップは<strong>任意（スキップ可）</strong>です。台帳づくりを楽にするための下準備なので、使わずに{' '}
      ...
      <section className="card space-y-3">
        ...
        {files.length > 0 && (
          <div className="space-y-2">
            ...
            <button type="button" className="btn-secondary text-xs" onClick={clearFiles}>
              すべてクリア
            </button>
          </div>
        )}
      ...
      {rows.length > 0 && (
        <section className="card space-y-4">
          ...
          <p className="text-xs text-slate-500">
            読み込んだ内容から<strong>日付・金額・摘要の列を自動推定</strong>しました。プレビューを確認し、違っていればプリセットや下の選択で調整してください。
          </p>

          <div className="flex flex-wrap gap-2">
            <span className="text-xs text-slate-500 self-center">プリセット:</span>
            {PRESETS.map((p) => (
              <button
                key={p.label}
                type="button"
          ...
          <div className="overflow-x-auto rounded border border-slate-200">
            <table className="min-w-full text-xs">
              <thead className="bg-slate-50 text-slate-500">
                <tr>
                  {colOptions.map((i) => (
                    <th key={i} className="border-b border-slate-200 px-2 py-1 text-left font-medium">
                      列{i}
                    </th>
                  ))}
                </tr>
              ...
              <tbody>
                {sampleRows.map((r, ri) => (
                  <tr key={ri} className={colMap.hasHeader && ri === 0 ? 'bg-amber-50 text-slate-400' : ''}>
                    {colOptions.map((i) => (
                      <td key={i} className="border-b border-slate-100 px-2 py-1 whitespace-nowrap">
                        {r[i] ?? ''}
                      </td>
                    ))}
                  </tr>
          ...
          <label className="flex items-center gap-2 text-sm text-slate-600">
            <input
              ...
              onChange={(e) => setColMap({ ...colMap, hasHeader: e.target.checked })}
            ...
            {([
              ...
              <div key={key}>
                ...
                <select
                  className="input"
                  value={colMap[key]}
                  onChange={(e) => setColMap({ ...colMap, [key]: Number(e.target.value) })}
                >
                  {colOptions.map((i) => (
      ...
      {candidates && (
        <section className="card space-y-3">
          <p className="label">3. サブスクを選んで取り込む（{candidates.length} グループ）</p>
          {candidates.length === 0 ? (
            ...
            <>
              <p className="text-xs text-slate-500">
                明細を店舗・サービス別にまとめました。
                <span className="font-medium text-slate-600">「毎月」「毎年」など定期課金らしいものは自動でチェック済み</span>
                です。実際に取り込むサブスクを目視で確認し、チェックを調整してください。
              </p>

              <div className="flex flex-wrap items-center gap-2 text-xs">
                <span className="text-slate-500">{selectedCount} 件を選択中</span>
                <button type="button" className="btn-secondary text-xs" onClick={() => setAllSelected(true)}>
                  表示中をすべて選択
                </button>
                <button type="button" className="btn-secondary text-xs" onClick={() => setAllSelected(false)}>
                  選択を解除
                </button>
                    ...
                    <li key={c.key} className="flex items-center gap-3 py-2">
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2">
                          <span className="truncate text-sm font-medium text-slate-800">{c.displayName}</span>
                        ...
                        <div className="text-xs text-slate-500">
                          ...
                          {c.dates.length > 0 && `・最終 ${formatDate(c.dates[c.dates.length - 1])}`}
                        </div>
                      </div>
                    </li>
                  );
              ...
              <button type="button" className="btn-primary" onClick={importSelected}>
